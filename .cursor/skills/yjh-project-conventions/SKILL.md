...
- **角色快照派生数值跨入口一致（V2.9 vitalsMax）**：顶栏要展示「当前/上限」→ 上限是派生值，必须在**所有返回角色快照的入口统一计算**（`/characters/me` 与 `/session/resume` 两处都返回 `vitalsMax`，勿只在一处算）；共用 game-core **同一函数**（`computeMaxVitals`/`maxFoodCapacity`，勿在各 service 重复实现公式）；需要的输入（内功等级 = `character_skills` 按 category=force 取 max、四维 attrs）在两处都查；测试用同公式重算核对数值（沿用结算自洽测试模式，characterService/sessionService 各补上限断言）。依赖内容包参数时工厂签名 `createXxxService(db, content?)` 保持 content 可选，无内容包（stub/单测）时派生值为 0 兜底。
- **时间驱动的状态结算（V2.12 / DC-044 / DC-051 自然恢复，对齐 xkx heal_up）**：随时间流逝的资源用 **`last_heal_at` + 交互入口 delta 结算**，不用定时任务——统一走 `settleCharacterVitals`（场景 getScene/move/act、`GET /characters/me`、`resume`）。规则在 game-core `applyRegen`（气精按 **绝对值** `con/3+maxNeili/10` 等 × `60/tickSeconds`，食水按 `foodPerMin`/`waterPerMin` 消耗；饥渴不回气精；须读写 `eff_qi`/`eff_jing`）；单次封顶 `maxWindowMinutes`；1 分钟内不结算。**新建角色必须写 `last_heal_at=now()`**（列 DEFAULT + INSERT 显式）；空值只初始化时钟，**切勿 `if (!last_heal_at) return` 永久跳过**（DC-044 实测根因）。客户端顶栏不会随场景响应自动变——须 `refreshVitals`（移动后 + 约 60s 轮询）。
