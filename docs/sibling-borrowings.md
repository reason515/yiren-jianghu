...
| xkx `app.css .vitals`（顶栏 grid 2 列，V2.10） | 顶栏生存项 2×2 网格两行排布（避免单行拥挤）：每项 小圆点/标签 + 数值，底部另起一行放食/饮 | V2.10 顶栏两行：气/精一行、精力/内力一行（当前/上限双值文本，非进度条）；银两徽章独立右侧胶囊（货币非状态） | 实测：两行网格 top 值分层、状态栏 53px，信息不再拥挤 |

**不借鉴**：xkx 胶囊圆角 chip（本项目保留矩形轻边框，已定组件语言）；xkx 顶栏纯进度条无数字（本项目保留当前/上限双值文本——文字叙事为主，数字只在需要时出现）；sanguo 汉末器物元素（题材不同）；双方文案一律原创。
