...
- **时间驱动的状态结算（V2.12 / DC-044 自然恢复，参照 pkuxkx heart_beat）**：随时间流逝的资源用 **`last_heal_at` + 交互入口 delta 结算**，不用定时任务——统一走 `settleCharacterVitals`（场景 getScene/move/act、`GET /characters/me`、`resume`）。规则在 game-core `applyRegen`（气精按上限比例恢复，食水按 `foodPerMin`/`waterPerMin` 绝对值消耗；同 delta 同输入同输出）；单次封顶 `maxWindowMinutes`；1 分钟内不结算。**新建角色必须写 `last_heal_at=now()`**（列 DEFAULT + INSERT 显式）；空值只初始化时钟，**切勿 `if (!last_heal_at) return` 永久跳过**（DC-044 实测根因）。客户端顶栏不会随场景响应自动变——须 `refreshVitals`（移动后 + 约 60s 轮询）。
