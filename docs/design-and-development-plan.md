...
| M3 H5 垂直闭环 | ✅ 完成 | E 阶段客户端全量 + H5 SPA 生产上线（http://117.72.34.43/，登录→建角→场景）；E14.1–E14.12 全部完成并发布（2026-08-08 生产部署：api/worker 新镜像 + 迁移 0010/0011 + H5 静态托管更新） |
...
- **✅ E14.5 战斗 UI**（依赖 F0）：CombatView 已升级为自动普攻 + 多敌同场（DC-037/038）：多血条、武侠战报、悬浮动作条（绝招/回气/逃跑）、进行中可收起、结束后离去；场景「较量」与断线 status 恢复均已接线。
...
- **✅ E14.8 地图与榜单**：MapSheet（区域地图八向导航）+ LeaderboardView（成长/赛季双榜）。服务端新增 `GET /map`（三件套：apiManifest/protocol/契约测试）返回内容包 rooms.grid + exits 去重无向边与当前所在标记；榜单为公开读，`isMe` 由客户端按自己的角色 id 标记。
  ...
  - 验收：地图可达区域可导航；榜单数据正确
- **✅ E14.9 断线重连体验**：ReconnectingOverlay + resume 恢复点 + 状态同步。网络层失败（fetch 抛错/非业务信封）自动进入重连（指数退避，复用 E12 状态机）；重连成功重新 resume 并刷新场景/战局/任务/挂机/榜单等全量状态，未读挂机战报自动翻开、断线期间完成的论剑直接弹回响；401 直接回登录，连超 5 次提示“久唤不应”。
  - 验收：断线重连回到场景且未读战报提示（状态机/DOM/服务端 resume E2E 覆盖）
  - 验收：断线重连回到场景且未读战报提示
- **✅ E14.10 体验打磨（美观度+易用性）**：tokens.css 视觉一致性复核（stat-exp/potential 等 token 已就位，未发现散落硬编码色值）；**ChoiceRow 分段控件泛型化落地**（禁 select，替换 AfkSheet/LeaderboardView/CharacterCreateSheet 三处分段）；**修复主界面底部导航 `.app-nav` 与全局 toast `.toast-host` 缺失样式的真实缺口**（此前导航/toast 无样式）；44px 触控与底部安全区（`--touch-min`/`--safe-b` 已在 tokens 与 sheet-scroll/app-nav 使用）。
  - 验收：mobile-ui 体验承诺——30 秒内知道“我在哪、有何变化、下一步”（导航/toast 可见性修复 + ChoiceRow 统一；DOM 单测覆盖）
...
- **🔄 E14.12 验收与发布**：各面板 DOM 单测补全 + 浏览器/真机全闭环（登录→建角→探索→学武→任务→战斗→挂机→PVP→论坛）+ 生产部署更新（重新 build:web + 上传）；**✅ check-doc-consistency 已移植**（`scripts/check-doc-consistency.js` + `pnpm test:docs-design`，CI quality 同步，design-docs §4 落地）。代码侧验收完成（350 单测 + 18 e2e + build:web 产物验证）；**✅ 生产部署已完成**（2026-08-08：`scripts/deploy-jd.cjs` 服务器本地构建 api/worker 镜像 + 迁移 0010/0011 + H5 dist 上传；生产冒烟全通：登录→建角→场景→地图→任务→赛季→挂机空态→移动→开战野狗→胜利结算→exp 落库）。浏览器/真机交互层验证按 checklist §3 清单在真机补充执行。
...
| D1–D8 切片内容 | ⬜ | | |
| D1 数值参数表 | ✅ 完成 | — | docs/numeric-baseline.md：pkuxkx 对照列归档（成长/状态/战斗/挂机/PVP/经济）+ 首版生效值 + 调整纪律；生效源已迁 `mechanics.yaml`（DC-046） |
| DC-046 机制公式 DSL | ✅ 完成 | — | `mechanics.yaml`（coeffs+formulas+piecewise+entityIndex）；安全 expr 引擎；game-core 求值接线；旧 params.json 退役 |
| 小数值重标 P0–P2 | ✅ 完成 | — | 历练门槛 level²×2；skill_power 压到等级量级；技能软顶 100/120；气血/伤害短打；NPC minExp 阶梯+中期奖励上调 |
| 机制迁移 A–D + E 回写 | ✅ 完成 | — | DC-047–049：后天四维/装备进战/招式 dodge；加力/伤势/运功子集；busy/hook/演示毒；任务相位加厚+传闻池+碧水渡第二区；mechanics/numeric-baseline/catalog/skill 回写 |
...
| E14 H5 面板闭环 | 🔄 进行中 | — | **E14.1 场景交互已接入**：交谈/请托/交易/拾取均从 EntitySheet 的结构化动作进入；ShopView 用服务端交易快照展示报价、银两与行囊，场景物品每角色仅可拾取一次（DC-025），真库 E2E 覆盖。**E14.2 角色面板与学武已接入**：`GET /characters/me` 提供角色四维、行止与有效潜能；客户端合并角色/武功/行囊快照，装备槽由已佩挂物品派生；请教/演练/参悟、佩上/卸下/使用均只提交服务端意图，结果刷新人物簿并反馈。**E14.3 任务面板已接入**：`GET /quests` 返回服务端组装的 `{ quests, story }`；QuestPanel 完成接/交/查、主线足迹、目标名与 goto 指向，移动抵达由服务端推进 goto 相位，战斗结束刷新 kill 进度；交差奖励即时回显。**E14.4 挂机闭环已接入**：`/afk/status`、`/afk/start`、`/afk/stop`、`/afk/reports` 与 resume 未读报告接入 GrindBanner/AfkSheet/AfkReportView；AfkSheet 以分段控件切换修炼/行侠，修炼选已学武功+时长，行侠选已接击杀差事+战术模板+时长，只提交受控意图（DC-026）。行侠作业由 Worker 固定种子自动结算，真库 E2E 覆盖。**E14.5 战斗 UI 已接入**：场景 NPC「较量」→ `/combat/start`，服务端状态/事件适配为 CombatView；受控 action（普攻/回气/逃跑/绝招）→ `/combat/action`，胜负横幅、收益摘要、断线 status 恢复均已接线。**E14.6 PVP 论剑已接入**：PvpView（赛季余日 + 对手列表）+ 发起对战二次确认 + PvpReplayView（匹配后拉取 `GET /pvp/matches/:id` 叙事回放，与 PVE 共用事件渲染器）。**E14.7 论坛完整交互已接入**：ForumView 板块→帖子→详情 + PostComposer 发帖/回帖/举报；点赞后用服务端返回值更新本地 likedByMe/计数。**E14.8 地图与榜单已接入**：服务端新增 `GET /map`（rooms.grid + 去重边 + current 标记）；MapSheet 相邻出口真实移动、跨房间提示；LeaderboardView 双榜接线，榜单公开读故客户端按角色 id 标记我的行。**E14.9 断线重连已接入**：网络失败自动进入指数退避重连（复用 E12 状态机），重连成功 resume 刷新全量状态并翻开未读挂机/论剑战报；401 回登录、超次提示。**E14.10 体验打磨已接入**：ChoiceRow 分段控件泛型化（替换 AfkSheet/LeaderboardView/CharacterCreateSheet）；修复 `.app-nav` 底部导航与 `.toast-host` 全局 toast 缺失样式的真实缺口；44px 触控与底部安全区已用 tokens。**E14.11 新手引导已接入**：GuideTip 轻提示 + 事件驱动首启动线（进场景/接任务/学武/首战），进度存 localStorage、乱序跳级不倒退（参考 sanguo first-session-ux-v3）。**E14.12 完成**：check-doc-consistency 移植（`pnpm test:docs-design`，CI quality 同步）；全部组件 DOM 单测覆盖；build:web 产物验证；**2026-08-08 生产部署完成**（`scripts/deploy-jd.cjs`：服务器本地构建 api/worker + 迁移 0010/0011 + H5 静态托管更新；生产 API 冒烟全链路通过）。浏览器/真机交互验证按 checklist §3 真机补充；借鉴 xkx ShopView/TrainSheet/FloatingPerfBar/ChoiceRow + sanguo first-session-ux（见 docs/sibling-borrowings.md）；估时 5–7 人日 |
| 登录页 V2.1（UI 增强） | ✅ 完成 | — | 登录页重做：水墨远景（孤月/疏星/三层空气透视山峦/月光洒落层/月下孤影持剑剪影+朱砂剑穗/松柏+朱砂灯笼/流雾，纯 CSS/SVG 无外部资源）+ 三卷竖排开场卷轴（只播一次可略过，localStorage `yjh.prologueSeen`）+ 文案重写（一帖一江湖 / 你听过的江湖都是别人的，这一扇门后是你的 / 叩门）；宣纸表单成对（输入框+叩门按钮）；手法借鉴 sanguo 登录徽记/开场画册（登记 sibling-borrowings §8）；技能沉淀 yjh-mobile-ui §4.13 + ui-visual-audit 多倍率截图/SVG 尺寸陷阱；Chrome 无头 + 视觉模型逐轮走查（登录 9.5/10）；94 h5 用例全绿 |
| V2.2 设计系统资产提炼 | ✅ 完成 | — | 登录页视觉提炼为可复用资产：`styles/atmosphere.css`（ink-* 远景原语 / paper-card 宣纸面±inset±rolls / v-cols 竖排 / 动效 keyframes 全集）+ tokens.css 补意境 token（--moon/--paper-surface/--wood/--lantern/--paper-ink）+ `docs/design-system.md`（token/原语/动效/组合示例/复用三步）；登录页与开场卷轴改为组合原语（重构后视觉零回归，截图对比验证）；yjh-mobile-ui §4.13 指向新资产；94 h5 用例全绿 |
| V2.3 建角流程改造 | ✅ 完成 | — | 建角从 Sheet 升级为全屏两步（`CharacterCreateSheet.tsx`）：①序章引导（ink-screen 远景 + 卷轴竖排五句交待"一人一江湖"背景 + 引导句 + 立名闯荡）→ ②立名与根基（印章 + 标题 + `.input.paper` 名号 + ChoiceRow 性别 + AttributeAllocator 四维 + `.btn.paper` 踏入江湖 + 回想序章）；宣纸控件原语 `.input.paper`/`.btn.paper`（auth.css）；atmosphere 补 ink-screen/ink-content 原语；修复 attr-row 1fr 溢出（minmax(0,1fr)）；补 4 个建角用例（序章→表单→提交→回想）；design-system §5 补建角组合示例；98 h5 用例全绿 |
| V2.4 文字颜色体系 | ✅ 完成 | — | 四维改水墨矿物色（赭石 #b06a42/黛蓝 #6b88ac/松绿 #76905f/琥珀 #a38c50，低饱和带墨感、色相隐喻）；数值字体规范（tabular-nums + 600 字重 + 领域色）；角色面板读数分色（当前值领域色/先天 paper-faint）；design-system §2.4 新增文字颜色三层体系（基础阶/语义强调/领域色矿物原则+现状表+迁移路线）；attr-hint 提亮、attr-btn 边框提亮；视觉模型：建角"沉稳国风、武侠沉浸感"、attr-card"色彩系统逻辑严密"；98 h5 用例全绿 |
| V2.5 四维统一墨色 | ✅ 完成 | — | **推翻 V2.4 四维四色**（用户反馈仍不符游戏风格、数字 19px 偏大）：确立"颜色只区分「类」不区分「项」"原则——四维属同类列表项，汉字即区分符号，统一 `--paper` 墨色 + 排版层级（display 属性名 16px / hint paper-dim 12px / 数值 600 tabular-nums 16px 不放大）；删 `--attr-*` 四色 token；角色面板读数改"当前值墨色加粗/先天值 faint 弱化"（不染色靠字重亮度）；design-system §2.4 升级类/项原则 + 领域色表改为物品/武功/状态/品质（🔄 待迁）；yjh-mobile-ui §4/§4.2 同步；视觉模型确认四行同色统一素雅；98 h5 用例全绿 |
| V2.6 起程过场 | ✅ 完成 | — | 建角后进入场景前新增剧情过渡（`DepartureOverlay.tsx`）：水墨远景 + 宣纸卡横排叙事——标题「老屋晨光」+ 两段正文（对齐内容包：village_start 硬板床醒来 / 村口武馆杂货铺客栈 / q_newbie_trail 野狗）+ 收束金句「江湖的第一步，往往从一件小事开始」+ 名字朱砂强调 + 「起身推门」墨锭按钮；App 接线（resume+数据就绪后置 departure，点击无缝进场景）；叙事纪律：过渡文案对齐内容包不编造；补 2 用例（渲染/回调）；yjh-mobile-ui 4.1 + 起程过场小节；视觉模型验证布局留白良好；100 h5 用例全绿 |
| V2.7 主界面重构 | ✅ 完成 | — | 主界面按设计系统重构：新增 `StatusBar` 顶栏生存状态条（气/精/精力/内力 + 银两，stat 色点 + tabular-nums，sticky 吸顶；数据接 resume/refreshCharacter 角色快照）；场景叙事前加「见闻」章回引首（gold 细线 + display 字体）；ExitPad 九宫格 260→190px 缩小（占用明显小于叙事区，玉色出口 + 中心房间名 + 按压态）；App 顶栏/场景页/底部导航整体视觉统一；补 2 用例（StatusBar 渲染/占位）；yjh-mobile-ui 4.1 + 4.12 主界面顶栏条目；视觉模型确认顶栏古朴可读、九宫格缩小美观；102 h5 用例全绿 |
| V2.8 主界面质感升级（对齐登录页） | ✅ 完成 | — | 主界面按登录页质感升级（DC-028）：①场景舞台背景——复用 atmosphere ink-* 原语弱化铺底（疏星/孤月/地平天光/远中两层山峦/流雾/暗角，fixed z-0 + 内容 z-1，叙事可读优先）；②顶栏生存条——铜金底边 + 发光 stat 点（8px + 同色 glow）+ 项间细分隔；③ExitPad——简牍内凹面板 + 空位方位字罗盘暗示（CSS ::before 伪元素，DOM/测试零改动）+ 玉色出口 glow + 铜金中心单行房名；④场景首字印章——纸纹噪点 + 内染辉光 + 微斜 -2deg（对齐 auth-seal 金石质感）；⑤「见闻」双侧墨线引首；⑥底部导航——渐变浮起 + 面板激活指示线（角色/挂机/任务/论剑/论坛/更多 on 态）；⑦quest-card 暖光纸纹；⑧token 层：`--stat-neili` 墨绿→黛紫 #8a7ab0（精力/内力同绿难辨，V2.8 二次提亮 #a08fce），气/精/精力同步提亮；视觉模型 1x/3x 多轮走查（终 8/10，罗盘可读、出口指向明确）；**踩坑沉淀**：vite dev（Windows）变换缓存会静默失效——部分 CSS 模块返回空 transform，截图前必须 fetch 验证 CSS 内容 + 重启 dev server（已入 ui-visual-audit 常见坑）；102 h5 用例全绿 |
| V2.9 主界面信息结构与滚动修复 | ✅ 完成 | — | 按用户 4 点反馈重构（DC-029）：①顶栏——生存项改「当前/上限」双值（`characterService`/`sessionService` 补 `vitalsMax`，与 sceneService 同一 `computeMaxVitals` 规则引擎，两服务单测各补上限断言 2 个）；银两改独立金色胶囊徽章（货币非状态，视觉明显区分）；②移除场景页「当前要事」大卡（开放世界玩家自行查任务列表）；③ExitPad 重构为只显示可前往方向的居中罗盘（北行上/南行下/中心当前房间，节约空间画面干净，方位语义保留）；④修复滚动 bug：`.scene` 底 padding 16→76px（fixed 导航遮挡 + 短视口无法滚动，实测 600px 视口滚动后底部内容可见）；Tab 文案「人物N」→「此地人物/此地物品/可做之事」+ 状态项分隔线提亮；验证：真实手机视口 DOM 探针 + 视觉模型（8.5/10，四需求全部落地），361 用例全绿 |
| V2.10 见闻动态流 + 顶栏两行 + 页签化 | ✅ 完成 | — | 按用户 3 点反馈重构（DC-030）：①顶栏生存项改两行 2×2 网格（气/精一行、精力/内力一行，银两徽章右侧居中，状态栏 53px）；②「见闻」重定义为互动动态流（参照 xkx EventLog）——场景描述（静态）留在标题下，新 `JournalFeed` 组件承接交谈/交易/拾取/战斗/交差/行止/移动等事件记录：折叠卡片（见闻标题 + 虚线分隔 + 最近 3 条摘要）→ 点击展开全屏可滚动历史（自动跟随底部、上滑翻看、'最新'按钮）；App 层 `addJournal` 挂钩全部互动点（talk 逐行带人名前缀/take 拾得/trade 买卖/combat 结果/quest 交差/afk 行止/移动）；③人物/物品/动作改页签式（无边框 + 选中玉色下划线 + 计数徽章 + `tab-panel` 内凹面板），不再像一排按钮；实测：交谈村长 → 对话逐行入见闻 → 展开翻看；362 用例全绿（新增 JournalFeed/页签用例） |
| V2.11 字体体系与按钮规范 | ✅ 完成 | — | 按用户 5 点反馈优化（DC-031）：①数字字体——引入 LXGW WenKai 数字子集（`--font-digit` 4.6KB，楷体笔意 + 全数字等宽 600），状态栏/银两/见闻数字统一（ZCOOL 数字不等宽 305–584、Noto 无楷韵）；②**button UA Arial 根治**：探针证实 `.journal-summary`/`.scene-tabs button` 是 button UA 默认字体（Arial）而非继承缺失——全局 `button,input,select,textarea { font-family: inherit }`；③见闻关键字高亮：人名前缀玉色 + 数字金色（`renderRich` 正则拆分，新增 2 用例）；④按钮统一：chip/exit-cell 分类 tint 渐变 + 内阴影 + 按压位移，方向钮 min-width 52→48px；⑤docs/design-system.md 新增 §2.2 字体体系（三字体用途矩阵+字号层级）与 §3.4 按钮规范；⑥数据链路核验：战斗扣血真实落库（qi 100→0），100/420 为「当前/上限」真实值；**遗留缺口**：`歇脚入眠`（canSleep）提示无服务端实现——待补 rest 恢复动作（未排期）；364 用例全绿（+2 JournalFeed） |
...
| V2.20 战斗条两行 + 场外运功（DC-052） | ✅ 完成 | — | 战斗底栏：上行加力分段+回气+逃跑，下行绝招横滑并美化；`POST /skills/exert` 场外疗伤/回气/回精；入口后改为人物簿状态页；新增静心回神 |
| 运功入口迁入状态页 | ✅ 完成 | — | 去掉顶栏右上「运功」；人物簿状态页行止下按已学自疗绝招显示对应按钮，未学不占位（DC-052 入口调整） |
...
**M3 H5 垂直闭环已收口**。**2026-08-13 玩家试玩审计 + DC-055 修复闭环已落地并经生产 R2 复核**（村环已通：0→1 请教豁免、开局佩剑、教学短打、鼠患目标、P1/廉价 P2）。**DC-056/057 武学页已落地**（三分页 + 境界 + 卸下 null + 数字同宋体）。**下一步**：G5 指标看板、M4 封测执行与周复盘（G4 持续任务）；城/门派历练 50/80 曲线另开。
