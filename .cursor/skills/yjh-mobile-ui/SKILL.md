...
- 地图：`GET /map`（鉴权）返回**当前区域** `rooms.grid` 与域内 `exits` 去重无向边（`areaId`/`areaLabel`），以及内容包 `maps/world.json` 组装的天下图；`state` 由服务端按角色位置/所在 area 标记；客户端只渲染，不重算网格/可达性。MapSheet 默认「本域」Tab，可切「天下」（点选远方只提示，不传送）。
- 场景「此刻可往」标题旁提供舆图入口；「更多」抽屉仍保留地图入口。
...
- **主界面顶栏生存状态（V2.7 落地 / DC-035）**：`StatusBar` sticky 吸顶，气/精/精力/内力 + 银两（stat 色点 + 语义标签 + tabular-nums，数据来自 resume/refreshCharacter 角色快照）；**整条可点打开人物簿（默认状态页）**；场景叙事前加「见闻」章回引首；ExitPad 九宫格 190px 缩小（占用比叙事区小，玉色出口 + 中心房间名），交互语义不变。
...
- **见闻增强与自然恢复（V2.12，DC-032）**：①见闻展开改**固定高度面板**（260px 超滚 + 收起按钮，替代全屏 Sheet——勿全屏打断阅读）；②**地名 mark 高亮**：移动/观察条目 `JournalEntry.mark` 关键词用青蓝色 `#8fb0c2`（jl-place），与人名玉色（jl-name）/数字金色（jl-num）三色区分（地名≠人名）；③**自然恢复**：对齐 xkx heal_up（DC-051 / DC-058）——`characters.last_heal_at` + 场景入口 delta 结算（`applyRegen`：绝对值 con/内力分母 × 9.5s 拍，封顶窗口防离线累积）；玩家被打残后走动/互动即较快回升；无饱腹/饮水条；④**观察动作**：NPC 内容包 `description` 字段（原创 wuxia 外观描述），`observe` 返回描述入见闻，EntitySheet 每项加「观察」；⑤**状态栏数字对齐**（已被 V2.13 细轨布局取代，见下条）。
- **观察 NPC 多行仪容（V2.16，DC-036）**：观察 NPC 时见闻串行多行——外形、武功水平、衣着/兵器（有 `equipment` 才补）；与档案仪容同档境界句。物品仍单行。`enqueue(lines)`，禁止把多行合成一条长 description 一次写入。
