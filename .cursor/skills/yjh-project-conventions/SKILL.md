...
- **发布流程（E14.12 实战，`scripts/deploy-jd.cjs`）**：`node scripts/deploy-jd.cjs explore|deploy`（密码从服务器信息文件读取不入库，ssh2 为根 devDep）。顺序：git archive 打包 → SFTP 上传 → 服务器本地 `docker build` api/worker → `API_IMAGE/WORKER_IMAGE=yiren/*:main compose up -d` → **迁移在新 api 容器内跑**（旧镜像不含新迁移文件，须先 up 新镜像再 `node node_modules/node-pg-migrate/bin/node-pg-migrate up`）→ H5 dist 上传 `/var/www/yiren`（先 `mv dist dist.bak`）。**生产冒烟业务 API 走 `/api/` 前缀**（Nginx `location /api/` 去前缀；`/health` 是特例直通）——用裸 `/auth/login` 会拿到 SPA HTML。
...
- **在线生计跑图（DC-045）**：在线沿 `hubRoomId`/`route` 逐步改 `room_path`，合圈发 `roundGain×onlineRewardMult`；先白名单导航到枢纽；挂机 running 禁手动 move。离线仍 hourly、不跑图。
...
**`.prettierignore` 只忽略 `docs/` 与 `pnpm-lock.yaml`——`.cursor/` 下的 skill/rules 与 `.pi/agent/AGENTS.md` 也受 format 门禁约束**，改过这些 markdown 记得 `pnpm format`（见常见坑 #30）。
...
30. **质量门禁失败定位与 agent 文档格式化（E14 收口后 CI run 125 踩过）**：
    ...
    - **`.cursor/` skill/rules 与 `.pi/agent/AGENTS.md` 参与 prettier 门禁**：`.prettierignore` 只忽略 `docs/` 与 `pnpm-lock.yaml`；新增/手改后必须 `pnpm format`，否则 CI `format:check` 挂（run 125 就是新增 skill 文档 + 改 AGENTS.md 未格式化）。
...
- **角色快照派生数值跨入口一致（V2.9 vitalsMax）**：顶栏要展示「当前/上限」→ 上限是派生值，必须在**所有返回角色快照的入口统一计算**（`/characters/me` 与 `/session/resume` 两处都返回 `vitalsMax`，勿只在一处算）；共用 game-core **同一函数**（`computeMaxVitals`，勿在各 service 重复实现公式）；需要的输入（内功等级 = `character_skills` 按 category=force 取 max、四维 attrs）在两处都查；测试用同公式重算核对数值（沿用结算自洽测试模式，characterService/sessionService 各补上限断言）。依赖内容包参数时工厂签名 `createXxxService(db, content?)` 保持 content 可选，无内容包（stub/单测）时派生值为 0 兜底。快照 vitals 仅气/精/精力/内力（DC-058，无饱腹/饮水）。
- **时间驱动的状态结算（V2.12 / DC-044 / DC-051 / DC-058 自然恢复，对齐 xkx heal_up）**：随时间流逝的资源用 **`last_heal_at` + 交互入口 delta 结算**，不用定时任务——统一走 `settleCharacterVitals`（场景 getScene/move/act、`GET /characters/me`、`resume`）。规则在 game-core `applyRegen`（气精按 **绝对值** `con/3+maxNeili/10` 等 × `60/tickSeconds`；须读写 `eff_qi`/`eff_jing`）；单次封顶 `maxWindowMinutes`；1 分钟内不结算。**无食水消耗、无饥渴 gate（DC-058）**。**新建角色必须写 `last_heal_at=now()`**（列 DEFAULT + INSERT 显式）；空值只初始化时钟，**切勿 `if (!last_heal_at) return` 永久跳过**（DC-044 实测根因）。客户端顶栏不会随场景响应自动变——须 `refreshVitals`（移动后 + 约 60s 轮询）。
