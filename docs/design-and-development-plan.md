...
| M3 H5 垂直闭环 | 🔄 进行中（核心闭环已接入） | E 阶段客户端全量 + H5 SPA 生产上线（http://117.72.34.43/，登录→建角→场景）；**剩余**：E14.11 新手引导、E14.12 验收发布 |
...
继续 **E14 H5 面板闭环**：E14.1–E14.10（场景/角色/任务/挂机/战斗/PVP/论坛/地图·榜单/断线重连/体验打磨）均已接入并验收。**下一步推进 E14.11 新手引导**（首启登录→建角→第一条任务→学武→首次战斗闭环，GuideTip 轻量提示；方法参考 sanguo first-session-ux-v3），随后 E14.12 验收与发布（试玩清单全绿 + 生产部署更新）。
