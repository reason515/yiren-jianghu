...
- `GET /characters/me`：角色快照含 `skillEnable`、`moves`、`performs`，以及各槎有效等级 `effective`（DC-056，与 `/skills/enable` / `/skills/mastery` 同口径；无内容包时为空对象）。客户端不得自算有效等级。
- `POST /skills/enable { slot, skillId }`（DC-041 / DC-057）：将特殊功挂到基本功槎位（`slot` ∈ force/dodge/parry/unarmed/sword/blade）。`skillId` 为 `null` 时写入显式清空（存档保留 `slot: null`），强制回退该槎基本功，**不会**再被 `autoEnableMap` 自动补回；仅当该槎从未配置（缺键）时解析才按 auto 补齐。服务端校验该特殊功 `enableSlots` 含此槎且已学（等级 > 0）。返回解析后的 `skillEnable` 全图（显式卸下处为 `null`）与各槎有效等级 `effective`。挂到槎上的特殊功等级越高，普攻越可能抽中其解锁招式（`pickMove`），未挂槎则该槎恒用基本功等级。
