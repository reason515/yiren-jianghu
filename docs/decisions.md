...
| DC-035 | 2026-08 | 人物簿四页签重梳（V2.15） | 人物簿从单卷长列表改为**固定摘要 + 四页签**（身势 / 武学 / 行囊 / 档案）：身势展示行止「当前/上限」细轨与四维；武学/行囊默认紧凑、点开再出动作；行囊页佩挂置顶；档案含性别、改名（`PUT /characters/name`）、放弃；顶栏 StatusBar 可点进人物（默认身势）；演练/参悟 toast 带花费与升段；武功展示演练点与描述。**请教归属与请教专项体验本轮不动** | 玩家实操：查生存/练功/配装被长列表淹没；对齐 uiux-v2「顶栏进人物」与 xkx 页签+展开节奏（不借 enable/命令） | docs/design-and-development-plan.md V2.15、yjh-mobile-ui §4.4、h5-client CharacterSheet/StatusBar | ✅ |
