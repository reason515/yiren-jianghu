...
| DC-034 | 2026-08 | 见闻新条目打字机显现（V2.14） | 见闻折叠卡与展开面板中，**仅首屏之后新追加的条目**以每批 2 字、约 32ms 间隔打字机显现，打字中带淡玉底与墨笔光标，打完褪去；**多行串行排队**（同时只打一行，打完再渲染下一行）；首屏历史立刻全文；遵守 `prefers-reduced-motion` | 用户反馈：新见闻出现不易察觉；多行并行推进不自然，应一行一行 | docs/design-and-development-plan.md V2.14、h5-client JournalFeed | ✅ |
