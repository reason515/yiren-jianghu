...
| M3 H5 垂直闭环 | 🔄 进行中（核心闭环已接入） | E 阶段客户端全量 + H5 SPA 生产上线（http://117.72.34.43/，登录→建角→场景）；**剩余**：E14.8 地图/榜单接线、E14.9–E14.12 重连/打磨/引导/验收 |
...
继续 **E14 H5 面板闭环**：E14.1–E14.7（场景/角色/任务/挂机/战斗/PVP/论坛）均已接入并验收。**下一步推进 E14.8 地图与榜单接线**（MapSheet 区域导航 + LeaderboardView 双轨榜，App 导航已就位待数据接线），随后 E14.9–E14.12 重连/打磨/引导/发布验收。
