...
- 物品 `items[]`：`kind` 决定装备槽位（weapon→weapon 槽、armor→armor 槽，**同槽替换**：新装自动卸下旧物；drug/food/misc 不可装备）；`usable`（消耗品效果，服务端结算）按 game-core 上限**钳制**——heal_qi/heal_jing 不超过 `eff_*`（DC-048）、cure_qi 抬高 `eff_qi`、restore_neili 以动态上限为顶，`amount` 是恢复量而非必达值；消耗后数量递减，最后一个删除（M2.5-inventory 实现）。
