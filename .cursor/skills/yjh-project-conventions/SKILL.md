...
- **时间驱动的状态结算（V2.12 自然恢复，参照 pkuxkx heart_beat）**：随时间流逝恢复的资源（qi/jing/neili）不用定时任务/轮询——**`last_heal_at` 时间戳 + 交互入口 delta 结算**（getScene/move/act 开头统一调用 `regenCharacter`），复用挂机 `last_tick_at` 思路；规则放 game-core 纯函数 `applyRegen`（确定性可测：同 delta 同输入同输出），服务端只做查库/写回；**单次封顶窗口**（maxWindowMinutes=30）防离线累积——离线不恢复是 MUD 惯例，但用窗口近似；1 分钟内不结算防高频；食水不自动恢复（饥饿机制未实现前勿混入）。新增迁移只加列不回填复杂值（`last_heal_at = now()` 兜底）。
