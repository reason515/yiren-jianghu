...
**第三层 · 领域色**（数据分类，用**水墨矿物色**）

设计原则：

1. **墨感**：色相取自中国画矿物颜料（赭石/黛蓝/松绿/琥珀/花青…），饱和度 ≤55%、明度 45–60%，保证墨底（#12100e）上可读且不跳脱；
2. **色相隐喻**：每域一色一义（如膂力=土铁厚重、悟性=文墨夜色），后续新增领域色先在此登记隐喻；
3. **双色纪律**：领域色只用于**标签与读数**（属性名/数值/类型名），正文说明文字永远用基础阶；
4. **文字标签并存**：品质/精通等不能只靠颜色，必须带文字（凡品/良品/珍品/名器）。

| 领域 | Token | 色值 | 色相隐喻 | 状态 |
|---|---|---|---|---|
| 四维·膂力 | `--attr-str` | #b06a42 | 赭石·土铁厚重 | ✅ V2.4 已迁 |
| 四维·悟性 | `--attr-int` | #6b88ac | 黛蓝·文墨夜色 | ✅ V2.4 已迁 |
| 四维·根骨 | `--attr-con` | #76905f | 松绿·竹木生机 | ✅ V2.4 已迁 |
| 四维·身法 | `--attr-dex` | #a38c50 | 琥珀·秋金轻灵 | ✅ V2.4 已迁 |
| 状态资源 | `--stat-*` | 见 tokens | 生存/修行资源 | 🔄 后续批次按矿物色对齐 |
| 武功门类 | `--skill-*` | 见 tokens | 武学分类 | 🔄 后续批次按矿物色对齐 |
| 物品/品质 | `--item-*` / `--quality-*` | 见 tokens | 物品种类/品级 | 🔄 后续批次按矿物色对齐 |

**数值字体规范**：

- 读数一律 `font-variant-numeric: tabular-nums`（分配/计数时数字不抖动）；
- 重要读数 `font-weight: 600`（Noto Serif 600 已自包）+ 领域色；
- 双数标明语义（当前/先天、当前/上限）：当前值着领域色，基准值用 `--paper-faint`。

**迁移路线**：V2.4 先迁四维（建角分配器 + 角色面板 attr-card）；状态/武功/物品/品质色按新界面或走查批次逐个对齐矿物色原则，每批截图对比防回归。
