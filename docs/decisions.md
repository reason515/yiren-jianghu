...
| DC-052 | 2026-08 | 场外运功子集（疗伤/回气/回精） | ①`POST /skills/exert`：已学自疗类绝招场外可用（heal / cure_ / heal_jing）；拒 damage/buff、拒战斗中。②无回合冷却，靠消耗与条件。③入口在人物簿状态页「行止」下：已学才显示对应运功按钮；未学不占位；武学页行内仍可运功；顶栏不再放独立入口。④新增 `force_calm_spirit` 回精。歇脚入眠仍另案 | 补 DC-051 场外缺口；不违 DC-041「无完整 exert 树」；顶栏独立钮与银两挤、未学也占位 | protocol、numeric-baseline、game-core/exert、skillsService、CharacterSheet、yjh-mobile-ui | ✅ |
